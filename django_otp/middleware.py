...
class OTPMiddleware(object):
    ...
    ``request.user.otp_device`` to the :class:`~django_otp.models.Device` object
    that has verified the user, or ``None`` if the user has not been verified.
    As a convenience, this also installs ``user.is_verified()``, which returns
    ...
    def process_request(self, request):
        ...
        device = Device.objects.from_persistent_id(device_id) if device_id else None
